...
export default function HistogramPage() {
  ...
  const [color2, setColor2] = useState("#ff6633");
  ...
  const validateConfig = (): { valid: boolean; message?: string } => {
    if (headers.length === 0) {
      return { valid: false, message: "No data loaded. Upload a CSV file first from the upload page, then return here to generate the histogram." };
    }
    if (!xField) {
    ...
    if (xField && columnTypes[xField] === "categorical") {
      return {
        valid: false,
        message: `Invalid X Field 1: "${xField}" is categorical. A histogram requires numeric data.\n\nWhat to do instead:\n- Select a numeric column (e.g., amount, price, age)\n- If you want counts per category, use Bar Chart or Pie Chart.`,
      };
    }
    if (xField2 && columnTypes[xField2] === "categorical") {
      return {
        valid: false,
        message: `Invalid X Field 2: "${xField2}" is categorical. A histogram comparison requires numeric data.\n\nWhat to do instead:\n- Select a numeric column for X Field 2\n- Or leave X Field 2 empty to plot a single numeric distribution.`,
      };
    }
    ...
    const binsVal = parseInt(bins, 10);
    if (isNaN(binsVal) || binsVal < 2 || binsVal > 500) {
      return { valid: false, message: "Bins must be between 2 and 500. Enter a valid integer." };
    }
    return { valid: true };
  };
  ...
  const generatePlot = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/univariate/histogram`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(parseApiError(err.detail ?? "Failed to generate histogram"));
      }
      ...
      if (msg.toLowerCase().includes("categorical") || msg.toLowerCase().includes("non-numeric")) {
        msg = msg.trim();
        if (!msg.endsWith(".")) msg += " ";
        msg += "Select numeric columns for the histogram.";
      } else if (msg.toLowerCase().includes("no csv") || msg.toLowerCase().includes("file uploaded")) {
        msg = "No CSV file loaded. Upload a CSV from the upload page first, then return here to generate the histogram.";
  ...
  const handleDownload = async () => {
    ...
    const plotTitle = xField2
      ? `Histogram_${xField}_vs_${xField2}`
      : `Histogram_${xField}`;
    try {
  ...
  return (
    <div className="histogram-container">
      ...
      <div className="config-panel">
        <h2 className="panel-title">Histogram Config</h2>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (v && columnTypes[v] === "categorical") {
                alert(
                  `You selected a categorical field for X Field 1.\n\n"${v}" is categorical, but Histogram needs a numeric field.\n\nSelect a numeric column instead (or use Bar Chart / Pie Chart for categorical data).`
                );
                setXField("");
                return;
              }
        ...
        <div className="form-group">
          ...
          />
          <span className="input-hint">Range: 5 – 100</span>
        </div>
      ...
      <div className="plot-area">
        {loading ? (
          ...
          <img src={plotUrl} alt="Histogram Plot" className="plot-image" />
        ) : (
